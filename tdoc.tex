...
Now type \texttt{git clone \string[url of project\string]} to download all the files of that project to your computer.
If you make a change and push it now you will be making changes to the master branch
Create a new branch by typing \texttt{git branch \string[new\string_branch\string_name\string]}
You can verify that the branch was created with \texttt{git branch}
And switch to this branch with \texttt{git checkout \string[new\string_branch\string_name\string]}
Make a change to a file, add it, and commit it.
\texttt{git add \string[filename\string]}
\texttt{git commit -m "commit message"}
You just committed the changes to your local branch.
You can push this branch to the repository with \texttt{git push -u origin \string[new\string_branch\string_name\string]}
